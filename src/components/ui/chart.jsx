  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          // 🔁 replaced token color utilities with literal HSL-based utilities
          "flex aspect-video justify-center text-xs " +
            "[&_.recharts-cartesian-axis-tick_text]:fill-[hsl(var(--muted-foreground))] " + /* 🔁 */
            "[&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-[hsl(var(--border))]/50 " + /* 🔁 */
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          // 🔁 border and bg converted to HSL-variables; text colors too
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-[hsl(var(--border))]/50 " +
            "bg-[hsl(var(--background))] px-2.5 py-1.5 text-xs shadow-xl", /* 🔁 */
          className,
        )}
      >
        {!nestLabel ? tooltipLabel : null}
              ...
              <div
                ...
                className={cn(
                  "flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 " +
                    "[&>svg]:text-[hsl(var(--muted-foreground))]", /* 🔁 */
                  indicator === "dot" && "items-center",
                )}
              >
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className="text-[hsl(var(--muted-foreground))]">{itemConfig?.label || item.name}</span> {/* 🔁 */}
                      </div>
                      {item.value && (
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-[hsl(var(--muted-foreground))]")} /* 🔁 */
          >
            {itemConfig?.icon && !hideIcon ? (
              <itemConfig.icon />
            ) : (
